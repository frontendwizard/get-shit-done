...
Then verify platform parameter is passed correctly:
```bash
node -e "const {getInstallPaths} = require('./dist/platform/install-adapter'); const oc = getInstallPaths(true, null, 'opencode'); console.log('OC commandsDir:', oc.commandsDir.endsWith('/command/gsd'))"
```
Expect: Output shows "OC commandsDir: true"
  </verify>
  <done>install.js updated with multi-platform TUI and --platform flag, platform parameter correctly passed to getInstallPaths()</done>
</task>
...
```
Expect: command/gsd/, opencode.json created
  </verify>
