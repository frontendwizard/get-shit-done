...
must_haves:
  truths:
    - "Workflows can detect agent completion without polling"
    - "Workflows can distinguish successful agents from failed agents"
    - "Workflows can await multiple agents asynchronously"
    - "Workflows can retrieve agent output after completion"
  artifacts:
    ...
    - path: "src/platform/index.ts"
      provides: "Exports agent instance classes"
      contains: "ClaudeCodeAgentInstance"
  key_links:
