...
- Requires TypeScript compilation from Plan 04 (install-adapter.ts is TypeScript)
...
Important: For local installs, bypass platform detection entirely (local mode is platform-agnostic, just uses ./.claude/).
  ...
  <verify>
Type-check the TypeScript source (compilation happens in Plan 04):
```bash
npx tsc --noEmit src/platform/install-adapter.ts
```
Expected: No type errors

Note: Runtime testing with `node test-install-adapter.js` happens after Plan 04 completes and TypeScript is compiled to dist/.
  </verify>
  <done>getInstallPaths() function exists and type-checks correctly (runtime verification after Plan 04)</done>
</task>
...
Note: Requires dist/ (compiled TypeScript output) not src/ (TypeScript source)
...
- [ ] TypeScript compiles install-adapter.ts to dist/platform/install-adapter.js
- [ ] bin/install.js requires install-adapter from dist/ (not src/)
...
- TypeScript compilation prerequisite satisfied
</success_criteria>
