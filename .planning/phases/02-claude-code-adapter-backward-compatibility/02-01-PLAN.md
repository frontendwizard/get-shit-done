...
must_haves:
  truths:
    ...
    - "Config read/write preserves existing settings"
    - "Adapter integrates with install.js without errors"
    - "Phase 4 methods (agent spawning, command unregistration) throw clear errors when called"
    - "settings.json is read and written correctly by adapter"
  artifacts:
...
- Has required readonly properties: `grep "readonly name:" src/platform/adapters/claude-code.ts`
- Has required readonly properties: `grep "readonly version:" src/platform/adapters/claude-code.ts`
