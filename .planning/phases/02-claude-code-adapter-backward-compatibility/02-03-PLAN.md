...
must_haves:
  truths:
    - Command files reference paths platform-agnostically
    - No hardcoded ~/.claude/ paths remain in command files
    - Commands work identically after path replacement during installation
  artifacts:
...
# Verify files without get-shit-done references are unchanged
git diff --stat commands/gsd/progress.md commands/gsd/add-todo.md
```
  </verify>
  <done>All 24 command files use {{config_dir}} where applicable, no hardcoded ~/.claude/get-shit-done/ paths remain</done>
</task>
