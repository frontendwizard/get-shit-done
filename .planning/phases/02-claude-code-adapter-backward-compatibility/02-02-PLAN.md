...
Test after refactor to ensure settings.json format unchanged.
  ...
  <verify>
```bash
# Build and verify adapter creation, async function signature, await calls, and config method invocation
npm run build && node --check bin/install.js && grep "async function install" bin/install.js && grep "await install" bin/install.js && grep -E "(readConfig|writeConfig|mergeConfig)" bin/install.js
```
  </verify>
  <done>install.js uses adapter.registerHook for hooks, uses adapter.getConfigDir/getCommandsDir/etc for paths, compiles without errors, syntax validates, async/await present, adapter config methods verified</done>
</task>
