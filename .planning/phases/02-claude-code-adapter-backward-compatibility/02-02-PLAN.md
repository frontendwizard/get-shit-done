...
<task type="auto">
  <name>Task 2: Refactor install.js to use adapter pattern with functional tests</name>
...
# Test adapter creation doesn't crash
node -e "const {createAdapter} = require('./dist/platform/registry'); const a = createAdapter(); console.log('Adapter ready:', a.name);"

# Syntax check install.js
node --check bin/install.js

# Verify async conversion (both syntactic and semantic)
grep 'async function install' bin/install.js && grep 'await adapter.registerHook' bin/install.js

# Verify adapter path methods are used
grep 'adapter.getConfigDir' bin/install.js && grep 'adapter.getCommandsDir' bin/install.js

# Functional test: Verify adapter methods are actually invoked
node -e "
const Module = require('module');
const originalRequire = Module.prototype.require;

let registerHookCalled = false;
let getConfigDirCalled = false;
  ...
  if (id.includes('platform/registry')) {
    return {
      createAdapter: () => ({
        ...
        version: '1.0.0',
        getConfigDir: () => {
          getConfigDirCalled = true;
          return '/tmp/test-config';
        },
        getCommandsDir: () => '/tmp/test-config/commands/gsd',
        getAgentsDir: () => '/tmp/test-config/agents',
        getHooksDir: () => '/tmp/test-config/hooks',
        registerHook: async (hookType, hookPath) => {
          registerHookCalled = true;
          console.log('✓ adapter.registerHook called:', hookType);
        },
        supportsHooks: () => true,
...
console.log('Adapter pattern verified - functional test confirms method invocation');
console.log('Full integration test in Plan 02-05');
"
```
  </verify>
  <done>install.js uses adapter.registerHook for hooks, uses adapter.getConfigDir/getCommandsDir/etc for paths, compiles without errors, syntax validates, async/await present, adapter method invocation verified via mock test</done>
</task>
...
- [ ] Async conversion verified (grep confirms async/await presence)
- [ ] Adapter method invocation tested via mock
</success_criteria>
