...
Test after refactor to ensure settings.json format unchanged.
  ...
  <verify>
```bash
# Build and verify adapter creation
npm run build && node --check bin/install.js && grep -q 'await adapter.registerHook' bin/install.js
```
  </verify>
...
- [ ] Async conversion verified (simpler check: build + syntax + grep)
</success_criteria>
