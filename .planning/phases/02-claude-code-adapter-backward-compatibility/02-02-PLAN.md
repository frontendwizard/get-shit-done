...
must_haves:
  truths:
    - install.js uses ClaudeCodeAdapter for all config/hook/command operations
  ...
  key_links:
    ...
    - from: bin/install.js
      to: ClaudeCodeAdapter.registerCommand
      via: "adapter.registerCommand(commandPath)"
      pattern: "adapter\\.registerCommand"
---
...
```
  ...
  <done>install.js uses adapter.registerHook for hooks, compiles without errors, syntax validates</done>
</task>
