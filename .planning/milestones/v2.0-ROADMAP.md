# Milestone v2.0: Multi-Platform GSD
...
- Hook registration data race (settings.json overwrites) — fixed with re-read pattern
- Configuration backup missing — added backupSettings() function
- OpenCode path structure confusion — corrected to command/gsd (singular)
- Test watch mode orphaning processes — changed npm test to single-run
...
- 9 commands require Claude Code Task tool (no OpenCode equivalent yet)
